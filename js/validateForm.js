...
function addCustomer() {
  ...
  var history_description = document.getElementById("history_description");
  var added_by = document.getElementById("added_by");
  var consultation_charges = document.getElementById("consultation_charges");
  ...
  if (!validateName(customer_name.value, "name_error")) {
  ...
  } else if (!validateAddress(history_description.value, "history_description_error")) {
    history_description.focus();
  } else {
    // Send AJAX request
    ...
    const params = new URLSearchParams({
      ...
      doctor_address: doctor_address.value,
      visit_date: visit_date.value,
      history_description: history_description.value,
      added_by: added_by.value,
      consultation_charges: consultation_charges.value || 0
    });

    xhttp.open("GET", "php/add_new_customer.php?" + params.toString(), true);
    xhttp.send();
  }
