...
function addInvoice() {
  ...
  if(!notNull(customers_name.value, "customer_name_error"))
    ...
    addNewInvoice(customers_name.value, customers_contact_number.value,invoice_number.value, invoice_date.value, total_amount.value, total_discount.value, net_total.value);
...
function addNewInvoice(customers_name, customers_contact_number, invoice_number, invoice_date, total_amount, total_discount, net_total) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (xhttp.readyState === 4 && xhttp.status === 200) {
  ...
  var params = 
    "action=add_new_invoice" +
    "&customers_name=" + encodeURIComponent(customers_name) +
    "&customers_contact_number=" + encodeURIComponent(customers_contact_number) +
    "&invoice_number=" + encodeURIComponent(invoice_number) + // ✅ added this line
    "&invoice_date=" + encodeURIComponent(invoice_date) +
    "&total_amount=" + encodeURIComponent(total_amount) +
    "&total_discount=" + encodeURIComponent(total_discount) +
    "&net_total=" + encodeURIComponent(net_total);

  xhttp.open("GET", "php/add_new_invoice.php?" + params, true);
  xhttp.send();
}
