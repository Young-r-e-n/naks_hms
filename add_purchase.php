...
<div class="col col-md-2 form-group">
  <label class="font-weight-bold" for="invoice_option">Invoice Number :</label>

  <!-- Dropdown to choose between entering a number or N/A -->
  <select class="form-control mb-2" id="invoice_option" onchange="toggleInvoiceInput(this.value)">
    <option value="custom" selected>Enter Invoice Number</option>
    <option value="NA">N/A</option>
  </select>

  <!-- Invoice number input -->
  <input type="text" class="form-control" placeholder="Invoice Number" id="invoice_number" name="invoice_number"
         onblur="invoiceBlurHandler(this.value);" />

  <code class="text-danger small font-weight-bold float-right" id="invoice_number_error" style="display: none;"></code>
</div>
<script>
function toggleInvoiceInput(option) {
  const input = document.getElementById('invoice_number');
  const error = document.getElementById('invoice_number_error');

  if (option === 'NA') {
    const uniqueSuffix = generateUniqueSuffix();
    input.value = `N/A-${uniqueSuffix}`;
    input.readOnly = true;
...
function generateUniqueSuffix() {
  // Format: YYYYMMDDHHMMSS (e.g. 20250703130545)
  const now = new Date();
...
function invoiceBlurHandler(value) {
  // Skip validation if it's a generated N/A invoice
  if (!value.startsWith('N/A')) {
    notNull(value, 'invoice_number_error');
    checkInvoice(value, 'invoice_number_error');
  } else {
    document.getElementById('invoice_number_error').style.display = 'none';
  }
}
</script>
          ...
          <div class="row col col-md-12 font-weight-bold">
            ...
            <div class="col col-md-1">Selling Price</div>
            <div class="col col-md-1">Buying Price</div>
