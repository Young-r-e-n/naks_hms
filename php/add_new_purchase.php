...
<!-- Expiry Date -->
<div class="col col-md-1">
  <select class="form-control mb-1" onchange="toggleExpiryInput(this, <?php echo $row_number; ?>)">
    <option value="custom" selected>Enter Expiry</option>
    <option value="12/40">12/40</option>
  </select>
  <input type="text" class="form-control" name="expiry_date" id="expiry_date_<?php echo $row_number; ?>">
  <code class="text-danger small font-weight-bold float-right" id="expiry_date_error_<?php echo $row_number; ?>" style="display: none;"></code>
      ...
      <script>
function toggleBatchInput(selectEl, rowNumber) {
  const input = document.getElementById(`batch_id_${rowNumber}`);
  if (selectEl.value === 'N/A') {
    input.value = 'N/A';
    input.readOnly = true;
    document.getElementById(`batch_id_error_${rowNumber}`).style.display = 'none';
  } else {
    input.value = '';
...
function toggleExpiryInput(selectEl, rowNumber) {
  const input = document.getElementById(`expiry_date_${rowNumber}`);
  if (selectEl.value === '12/40') {
    input.value = '12/40';
    input.readOnly = true;
    document.getElementById(`expiry_date_error_${rowNumber}`).style.display = 'none';
  } else {
    input.value = '';
    input.readOnly = false;
  }
